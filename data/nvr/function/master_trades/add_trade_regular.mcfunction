...
$execute if items entity @n[type=item,tag=trade.item] container.* #nvr:single_item_trades run \
...
$execute unless items entity @n[type=item,tag=trade.item] container.* #nvr:single_item_trades unless items entity @n[type=item,tag=trade.item] container.* minecraft:enchanted_book run \
data modify entity @s Offers.Recipes append value \
